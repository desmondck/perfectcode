...
关于Checkpoint，引用《状态机Checkpoint详解》中的前两段：
...
>
> Checkpoint的出现用于解决这个问题。我们不能要求开发者做到时刻的状态机数据正确，但要求开发者生成一个状态机的Checkpoint的时候，这份数据必须是正确的完整的。一个Checkpoint代表着一份某一时刻被固化下来的状态机数据，它通过sm.h下的StateMachine::GetCheckpointInstanceID\(\)函数反馈它的精确时刻，于是每次启动的重演，我们只需要从这个时刻所指向PaxosLog位置开始而不是从0开始。
